...
- 本リポジトリの AnyREAL 実装（`src/`, `scripts/`, `tests/`, `docs/`）は MIT License（`LICENSE`）。
- `patches/` は上流 REAL のファイルへの差分であり、**本リポジトリの MIT ライセンス対象外**。
  上流の権利表示を保持し、上流のライセンス条件に従う。参照 commit の時点で上流に
  LICENSE/COPYING は確認できていないため、再配布・公開の条件は別途確認が必要。
- 上流コードそのものは本リポジトリに含めない（`third_party/` は `.gitignore`）。
